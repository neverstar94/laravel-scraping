...
userdata_path = "C:\\Users\\Never\\AppData\\Local\\Google\\Chrome\\User Data\\Profile6"
options = webdriver.ChromeOptions() 
options.add_argument("user-data-dir={}".format(userdata_path))
driver = webdriver.Chrome(executable_path="chromedriver", options=options)
driver.get("https://www.qfc.com/pr/boost")


def main():
	mycursor = connection.cursor(buffered=True)
	mycursor.execute("SELECT * FROM siteinfos WHERE url = 'Qfc'")
...
def test_chrome(userid,useremail,userpassword):
	...
	time.sleep(5)
	driver.get("https://www.qfc.com/signin?redirectUrl=/")
	
	time.sleep(5)
	username = driver.find_element(By.ID, "SignIn-emailInput")
	password = driver.find_element(By.ID, "SignIn-passwordInput")
	login = driver.find_element(By.ID, "SignIn-submitButton")
